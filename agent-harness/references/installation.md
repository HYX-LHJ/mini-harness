...
```bash
npx skills add <owner>/round-harness --list
npx skills add <owner>/round-harness --skill agent-harness -g -y   # 全局
npx skills add <owner>/round-harness --skill agent-harness -y      # 项目
npx skills add <owner>/round-harness --skill agent-harness -a cursor -a claude-code -a codex -g -y
```
...
Portable skill — works on Cursor, Codex, Claude Code, and any agent that loads `SKILL.md` directories. Copy `agent-harness/` to the tool-specific path above, or use `npx skills add <owner>/round-harness@agent-harness`. See [docs/installation.md](../../../docs/installation.md).
