...
skipped by default (use ``--force`` to overwrite).

Usage (from the **repository to initialize** root, or with ``--root``)::

    python <skill-dir>/scripts/init_harness.py --root .
    python <skill-dir>/scripts/init_harness.py --root . --project-name my_api

Standard Agent invocation: see skill ``SKILL.md`` "Create harness".

Examples:
    Input: empty git repo + --root .
    Output: full harness/ tree, AGENTS.md, pytest.ini, three gate scripts
"""
...
def init_harness(
    ...
    *,
    ...
    """Generate standard harness under ``root``; return list of verification misses."""
    today = date.today()
    ...
    text_templates: dict[Path, Path] = {
        ...
        TEMPLATES / "docs-weekly-review.md": harness / "docs" / "weekly-review.md",
    ...
    backlog_files = {
        harness / "backlog" / "archive.md": "# Historical weekly tasks\n\n(Cross-week archive from todo.md)\n",
        harness / "backlog" / "decisions-archive.md": "# Archived decisions\n\n",
    }
